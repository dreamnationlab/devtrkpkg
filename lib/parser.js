module.exports = {
  ...
  fedexGetYear:function(timeStamp){
      let TIMESTAMP = new Date(timeStamp);
      return TIMESTAMP.getFullYear();
  },
  fedexGetMonth:function(timeStamp){
      let TIMESTAMP = new Date(timeStamp);
  ...
  fedexGetDate:function(timeStamp){
      let TIMESTAMP = new Date(timeStamp);
      return TIMESTAMP.getDate();
  },
  fedexGetDay:function(timeStamp){
      let TIMESTAMP = new Date(timeStamp);
      return this.getDay(TIMESTAMP.getDay());
  },
  fedexGetHours:function(timeStamp){
      let TIMESTAMP = new Date(timeStamp);
      return TIMESTAMP.getHours();
  },
  fedexGetMinutes:function(timeStamp){
      let TIMESTAMP = new Date(timeStamp);
  ...
  FEDEX:function(resJson, callback){
    ...
    if(TRACKDETAILS[0].DatesOrTimes){
      for(let i = 0, length = TRACKDETAILS[0].DatesOrTimes.length; i < length; i++){
        if(TRACKDETAILS[0].DatesOrTimes[i].Type === 'ESTIMATED_DELIVERY'){
          card.exp_dv.yy = this.fedexGetYear(TRACKDETAILS[0].DatesOrTimes[i].DateOrTimestamp);
          card.exp_dv.mm = this.fedexGetMonth(TRACKDETAILS[0].DatesOrTimes[i].DateOrTimestamp);
          card.exp_dv.dd = this.fedexGetDay(TRACKDETAILS[0].DatesOrTimes[i].DateOrTimestamp);

          let ampm = this.getAMPM(this.fedexGetHours(TRACKDETAILS[0].DatesOrTimes[i].DateOrTimestamp));

          card.exp_dv.time = this.fedexGetHours(TRACKDETAILS[0].DatesOrTimes[i].DateOrTimestamp) +
                            ':' +
                            this.fedexGetMinutes(TRACKDETAILS[0].DatesOrTimes[i].DateOrTimestamp) +
                            ' ' + ampm;
          break;
        }
      }
    ...
    if(TRACKDETAILS[0].Events){
      ...
      let dada = new Date(EVENTS[0].Timestamp);
      console.log(dada.getUTCHours());
      console.log(dada.getUTCMinutes());
      console.log(this.fedexGetHours(EVENTS[0].Timestamp));
      console.log(this.getDay(EVENTS[0].Timestamp));

      let ampm = this.getAMPM(this.fedexGetHours(EVENTS[0].Timestamp));
      //상태요약_요일시간 :: April 21,2018 at 8:00 PM
      card.summary.datetime = this.fedexGetMonth(EVENTS[0].Timestamp) +
                              ' ' +
                              this.fedexGetDate(EVENTS[0].Timestamp) +
                              ', ' +
                              this.fedexGetYear(EVENTS[0].Timestamp) +
                              ' at ' +
                              this.fedexGetHours(EVENTS[0].Timestamp) +
                              ':' +
                              this.fedexGetMinutes(EVENTS[0].Timestamp) +
                              ' ' + ampm;
      ...
      if(EVENTS[0].Address){
        ...
        if(EVENTS[0].StateOrProvinceCode){
          card.summary.place += ', ' + EVENTS[0].StateOrProvinceCode;
        }
        if(EVENTS[0].CountryName){
          card.summary.place += ', ' + EVENTS[0].CountryName;
        }
      }
      ...
      if(card.eventnum === 5){
          ...
          card.exp_dv.yy = this.fedexGetYear(EVENTS[0].Timestamp);
          card.exp_dv.mm = this.fedexGetMonth(EVENTS[0].Timestamp);
          card.exp_dv.dd = this.fedexGetDate(EVENTS[0].Timestamp);
          card.exp_dv.time = this.fedexGetHours(EVENTS[0].Timestamp) +
                            ':' +
                            this.fedexGetMinutes(EVENTS[0].Timestamp) +
                            ' ' + ampm;
      }
      ...
      for(let i=0, length=EVENTS.length; i < length ; i++) {
        let ampm = this.getAMPM(this.fedexGetHours(EVENTS[i].Timestamp));
        let history = {
            "datetime":this.fedexGetMonth(EVENTS[i].Timestamp) +
                      ' ' +
                      this.fedexGetDate(EVENTS[i].Timestamp) +
                      ', ' +
                      this.fedexGetYear(EVENTS[i].Timestamp) +
                      ' at ' +
                      this.fedexGetHours(EVENTS[i].Timestamp) +
                      ':' +
                      this.fedexGetMinutes(EVENTS[i].Timestamp) +
                      ' ' + ampm,
            "status":"",                              //상태
            "place":"",                               //장소
          ...
          if(EVENTS[i].Address){
            ...
            if(EVENTS[i].StateOrProvinceCode){
              history.place += ', ' + EVENTS[i].StateOrProvinceCode;
            }
            if(EVENTS[i].CountryName){
              history.place += ', ' + EVENTS[i].CountryName;
            }
          }
